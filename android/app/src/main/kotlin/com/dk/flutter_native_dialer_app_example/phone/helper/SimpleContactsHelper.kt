...
import com.dk.flutter_native_dialer_app_example.phone.core.PERMISSION_READ_CONTACTS
import com.dk.flutter_native_dialer_app_example.phone.extensions.hasPermission
import com.dk.flutter_native_dialer_app_example.phone.extensions.queryCursor
...
import com.dk.flutter_native_dialer_app_example.phone.models.SimpleContact.Companion.normalizeRegex
...
class SimpleContactsHelper(val context: Context) {
    ...
    // Danh sách màu nền để tạo biểu tượng chữ cái
    ...
    // Phương thức truy xuất danh bạ điện thoại và trả về danh sách các liên hệ đơn giản ('SimpleContact').
    // Nhận đối số favoritesOnly: chỉ định chỉ lấy các liên hệ yêu thích hay tất cả các liên hệ.
    // callBack: sử dụng để xử lý danh sách liên hệ sau khi nó được lấy.
    ...
    // Đây là một phương thức để lấy thông tin liên quan đến tên và hình ảnh từ danh bạ điện thoại.
    // Phương thức này nhận đối số favoritesOnly để chỉ định xem chỉ lấy các liên hệ yêu thích hay
    // tất cả các liên hệ.
    ...
    //Đây là một phương thức để lấy thông tin liên quan đến số điện thoại từ danh bạ điện thoại
    // Trả về danh sách các số điện thoại: ArrayList
    ...
    private fun getNameLetter(name: String) = Normalizer.normalize(name, Normalizer.Form.NFD).replace(normalizeRegex, "").toCharArray().getOrNull(0)?.toString()?.uppercase(
        Locale.getDefault()
    ) ?: "A"
...
//    }
    ...
    private fun String.repeatString(n: Int): String {
        return buildString {
            repeat(n) {
                append(this@repeatString)
            }
        }
    }

    private fun getQuestionMarks(size: Int) = "?,".repeatString(size).trimEnd(',')
